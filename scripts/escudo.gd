...
@onready var ir: AudioStreamPlayer2D = $ir
@onready var voltar: AudioStreamPlayer2D = $voltar

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	self.scale = Vector2(0.1, 0.1)
	get_tree().create_tween().tween_property(self, "scale", Vector2(1, 1), 0.8)
	ir.play()
	timer.start()

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(_delta: float) -> void:
	get_tree().create_tween().tween_property(self, "global_position", get_parent().get_child(2).global_position, 0.35)
